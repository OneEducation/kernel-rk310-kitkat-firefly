...
//box
	// if (0/*(rate <= (297*MHZ)) && (gpll_rate%rate == 0)*/) {
		*best_parent_p = npll;
		best = rate;
		*best_parent_rate = npll->rate;
		printk("line %d of func %s\n", __LINE__, __func__);
//	} else {
//		*best_parent_p = cpll;
//		div = RK3288_LIMIT_PLL_VIO0/rate;
//		prate = div * rate;
//		*best_parent_rate = clk_round_rate(cpll, prate);
//		*best_parent_rate = 594*MHZ;
//		
//		best = (*best_parent_rate)/div;	
//	}

	printk("%s best_parent_rate == %lu\n", __func__, *best_parent_rate);
	printk("%s best == %lu\n", __func__, best);
	return best;
}
		...
		unsigned long parent_rate)
{
	...
	struct clk* parent;
	unsigned long check_rate;

	printk("%s rate == %lu\n", __func__, rate);
	printk("%s, line %d: parent == %lu\n", __func__, __LINE__, parent);
	printk("%s, line %d: parent_rate == %lu\n", __func__, __LINE__, parent_rate);

    clk_divider_ops.set_rate(hw, rate, parent_rate);

	/* set aclk_vio */
	// if (/*parent_rate == __clk_get_rate(gpll)*/0) {
		parent = clk_get(NULL, "clk_npll");
		printk("%s, line %d\n", __func__, __LINE__);
		clk_set_parent(aclk_vio0, npll);
		//clk_set_rate(aclk_vio0, 300*MHZ);
		clk_set_rate(aclk_vio0, __clk_get_rate(parent));
		check_rate = __clk_get_rate(parent);
		printk("line %d of func %s: check_rate == %lu\n", __LINE__, __func__, check_rate);
//	} else
